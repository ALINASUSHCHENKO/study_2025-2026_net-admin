...
\item Посмотрели журнал событий (Рис. \ref{04}-\ref{05}):
\begin{minted}{bash}
    tail -f /var/log/fail2ban.log
\end{minted}
...
\item Посмотрели журнал событий (Рис. \ref{06}):
\begin{minted}{bash}
    tail -f /var/log/fail2ban.log
\end{minted}
\end{enumerate}
...
\begin{enumerate}
\item На сервере посмотрели статус {\tt fail2ban} (Рис. \ref{07}):
\begin{minted}{bash}
...
\item Посмотрели статус защиты SSH и {\tt fail2ban} (Рис. \ref{08}):
\begin{minted}{bash}
...
\item Установили максимальное количество ошибок для SSH, равное 2 (Рис. \ref{09}):
\begin{minted}{bash}
...
\item С клиента попытались зайти по SSH на сервер с неправильным паролем (Рис. \ref{10}).
...
\item На сервере посмотрели статус SSH (Рис. \ref{11}):
\begin{minted}{bash}
    fail2ban-client status sshd
\end{minted}

\item Убедились, что произошла блокировка адреса клиента (Рис. \ref{11}).
...
\item Разблокировали IP-адрес клиента (Рис. \ref{12}):
\begin{minted}{bash}
    fail2ban-client set sshd unbanip 192.168.1.30
\end{minted}

\item Вновь посмотрели статус защиты SSH (Рис. \ref{12}):
\begin{minted}{bash}
    fail2ban-client status sshd
\end{minted}
  ...
  Убедились, что блокировка клиента снята (Рис. \ref{13}):
...
\item Перезапустили {\tt fail2ban}.
\item Посмотрели журнал событий (Рис. \ref{14}):
\begin{minted}{bash}
    tail -f /var/log/fail2ban.log
\end{minted}
...
\item Вновь попытались войти с клиента на сервер с неправильным паролем (Рис. \ref{15}).
  ...
  \end{center}
   \begin{center}
    ...
    \captionof{figure}{Проверка работы ignore.}
    \label{15}
  \end{center}
...
\begin{enumerate}
\item На виртуальной машине server перешли в каталог для внесения изменений в настройки внутреннего окружения {\tt /vagrant/provision/server/}, создали в нём каталог {\tt protect}, в который поместили в соответствующие подкаталоги конфигурационные файлы 
\begin{minted}{bash}
...
\item В терминале {\tt /vagrant/provision/server} создали исполняемый файл {\tt protect.sh}
